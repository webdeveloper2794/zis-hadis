...
const Page = () => {
  ...
  return (
    <div className="w-full py-1 px-4 bg-transparent rounded-md">
      <h1 className="text-center font-bold text-lg text-green-900 capitalize">
        Habarlar 
      </h1>
      <div className="overflow-x-auto py-8 max-w-screen-lg m-auto">
        {errorReport.length === 0 ? (
          <div className="text-center text-gray-500 text-lg capitalize">Hech qanday habar yo&apos;q!</div>
        ) : (
          errorReport.map((error, index) => (
            <main
              className="bg-white shadow-xl p-2 text-left w-full rounded-md my-4"
              key={error._id}
            >
              <Link
                href={`/admin/book/${error.bookId}/chapter/${error.chapterId}/hadith/${error.hadithId}`}
              >
                <button className="btn btn-outline btn-success  group mx-2">
                  <FaEdit className="group-hover:text-white" />
                </button>
              </Link>
              <button
                onClick={() => handleRemove(error._id)}
              ...
              <article className="flex flex-col md:flex-row justify-between items-start gap-2">
                <div
                  role="tablist"
                  className="tabs tabs-boxed bg-white text-gray-800 w-full md:w-1/2 max-w-[40rem]"
                >
                  <input
                    type="radio"
                    name={`my_tabs_${index}`}
                    role="tab"
                    className="tab text-gray-800 ring-1"
                  ...
                  <>
                    <div className="p-2 max-w-[40rem] w-full md:w-1/2 pt-1 flex flex-col">
                      <p className="text-lg text-red-600 text-center font-semibold">
                        Habar <MdOutlineReportGmailerrorred />
                      </p>
