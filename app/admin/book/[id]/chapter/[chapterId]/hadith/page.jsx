            ...
            <div>
                ...
                {hadiths.sort((a, b) => a.subChapterNumber - b.subChapterNumber).map((hadith, index) => (
                    <div key={hadith._id}>
                        ...
                        <Link href={`/admin/book/${id}/chapter/${chapterId}/hadith/${hadith._id}`} >
                                        <button className="btn btn-outline btn-success  group"><FaEdit className="group-hover:text-white" /></button>
                                    </Link>
                            <button onClick={() => handleDelete(hadith._id)} className="btn btn-outline btn-error group"><MdDeleteSweep className="group-hover:text-white" /></button>
                        </section>
                        <section className='flex flex-wrap justify-between my-4 '>
                            <h1 className='text-base'><span className='m-2'>{hadith.subChapterNumber}.Bob: </span>{hadith.subChapterTitle.uz}</h1>
                            <h1 className='text-lg text-right '> {hadith.subChapterTitle.ar} :باب</h1>
