...
const Page = ({ params }) => {
  ...
  return (
    <div className="w-full py-10 px-4  rounded-md">
      <h2 className="text-xl font-bold mb-2 text-green-700 text-center">
        Mundarija
      </h2>
      <div className="overflow-x-auto py-10 w-full m-auto">
        <table className="table bg-transparent">
          <thead>
            <tr>
              <th className="text-green-900 text-lg font-semibold hover:text-green-500">
                #
              </th>
              <th className="text-lg text-green-900 font-semibold">Bob</th>
              <th className="text-lg text-green-900 font-semibold">باب</th>
              <th className="text-lg text-green-900 font-semibold">Tahririlash</th>
              <th className="text-lg text-green-900 font-semibold">O&apos;chirish</th>
              <th className="text-lg text-green-900 font-semibold">Qo&apos;shish</th>
            </tr>
          </thead>
          <tbody>
            {sortedChapters.map((chapter, index) => (
              <tr
                key={chapter._id}
                className={`bg-white/50  hover:bg-green-100 transition-colors duration-300 cursor-pointer ${
                  index % 2 === 0 ? "bg-green-50" : "bg-green-100"
                }`}
              >
                <td className="text-green-900 ">{chapter.chapterNumber}</td>
                <td className="text-green-900 md:text-lg font-semibold group">
                  <Link
                    href={`/admin/book/${id}/chapter/${chapter._id}/hadith`}
                    className="group-hover:text-green-500 flex items-center"
                  >
                    <HiChevronRight className="group-hover:text-green-500" />
                    {chapter.title_uz}
                  </Link>
                </td>
                <td className="text-green-900 md:text-lg font-semibold">
                  {chapter.title_ar}
                </td>
                ...
                <td className="text-green-900">
                  <button
                    onClick={() => handleRemove(chapter._id)}
                ...
                <td className="text-green-900">
                  <Link href={`/admin/addchapter`}>
                    <button className="btn btn-outline btn-info group">
                      <MdOutlinePostAdd className="group-hover:text-white" />
                    </button>
                  </Link>
                </td>
              </tr>
